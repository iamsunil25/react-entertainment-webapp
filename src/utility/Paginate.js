...
<nav aria-label="Page navigation example">
  <ul className="inline-flex -space-x-px">
    ...
    <li   style={{pointerEvents:page=='1' ? 'none':'', cursor:page=='1' ? 'default':'pointer'}}  onClick={previousPage}  className="px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</li>
	{
		 Array.from({length:30},()=>'filled').map((item,index)=><li key={index+1} onClick={()=>setPage(index+1)} style={{cursor:'pointer'}}  className={`px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${page===index+1 ? 'activePage':''}`}>{index+1}</li>)
	}
