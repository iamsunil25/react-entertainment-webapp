...
onClick={()=> {
 ...
 key={item.to} >
          <Link style={{color:'#6b7280'}} to={item.to} className={`block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0  md:p-0 dark:text-gray-400  dark:hover:bg-gray-700 md:dark:hover:bg-transparent  ${pathname===item.to  || pathname.includes("tvseries") && item.routeName.includes("TV") || pathname.includes("movies") &&  item.routeName.includes("Movies") ? "text-blue-700":""}` }>{item.routeName}</Link>
        </li>
})
	}
      </ul>
